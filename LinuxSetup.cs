    ...
    private static void checkInstallSiegfried() 
    {
        ...
        if (!output.Contains("Siegfried"))
        {
            ...
            if(r == "Y")
            {
                ...
                startInfo2.Arguments = "-c \"su -c 'apt install curl && curl -sL http://keyserver.ubuntu.com/pks/lookup?op=get&search=0x20F802FE798E6857 | gpg --dearmor | tee /usr/share/keyrings/siegfried-archive-keyring.gpg && echo \'deb [signed-by=/usr/share/keyrings/siegfried-archive-keyring.gpg] https://www.itforarchivists.com/ buster main\' | tee -a /etc/apt/sources.list.d/siegfried.list && apt-get update && apt-get install siegfried'\"";
